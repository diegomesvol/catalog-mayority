      ...
      <main className="mx-auto w-full max-w-3xl flex-1 px-4 py-6 sm:px-6 sm:py-8">
        ...
        {pedidos.length === 0 ? (
          <p className="text-sm text-ink-500">
            Todavía no tenés pedidos registrados. Los pedidos que envíes por WhatsApp desde el catálogo van a aparecer acá.
          </p>
        ) : (
          <ul className="flex flex-col gap-3">
            {pedidos.map((pedido) => (
              <li key={pedido.id} className="rounded-2xl border border-ink-200 bg-paper-raised p-4">
